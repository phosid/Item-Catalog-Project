...
from database_setup import Base, AnimeShow, Genre
...
#@app.route('/catalog/<int:genre_id>/edit/', methods=['GET', 'POST'])
...
def animeCatalog(genre_id):
	...
	shows = session.query(AnimeShow).filter_by(genre_id = genre.id)
	return render_template('animeshow.html', genre = genre, shows = shows)
...
def newAnimeShow(genre_id):
	...
	genre = session.query(Genre).filter_by(id=genre_id).one()
	if request.method == 'POST':
		newItem = AnimeShow(name = request.form['name'], year = request.form['year'], description = request.form['description'], genre_id = genre_id)
		session.add(newItem)
		session.commit()
